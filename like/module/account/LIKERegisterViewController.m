...
@interface LIKERegisterViewController () <UITableViewDataSource, UITableViewDelegate, LIKERegionViewControllerDelegate> {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = NSLocalizedStringFromTable(@"countryAndArea", LIKELocalizeAccount, nil);
    cell.textLabel.textColor = [UIColor darkGrayColor];
    
    if (_countryAndAreaCode) {
        cell.detailTextLabel.text = _countryAndAreaCode.countryName;
    }
    else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self performSegueWithIdentifier:@"regionSegue" sender:self withBlock:^(id sender,
                                                                            UINavigationController *destinationVC,
                                                                            UIStoryboardSegue *segue) {
        LIKERegionViewController *regionViewController = (LIKERegionViewController *)[destinationVC topViewController];
        [regionViewController setAreaArray:_areaArray];
        regionViewController.delegate = self;
    }];
}

#pragma mark LIKERegionViewControllerDelegate

- (void)setCountryAndAreaCode:(CountryAndAreaCode *)countryAndAreaCode {
    _countryAndAreaCode = countryAndAreaCode;
    NSLog(@"the area data：%@,%@", countryAndAreaCode.areaCode,countryAndAreaCode.countryName);
    
    self.areaCodeTextField.text = [NSString stringWithFormat:@"+%@", _countryAndAreaCode.areaCode];
...
- (IBAction)nextbarButtonClick:(id)sender {
    ...
    if ([LIKEHelper verifyPhoneNumber:self.phoneNumberTextField.text]) {
        ...
        // 去掉 area code 前面的 "+"
        [LIKEUserContext sharedInstance].tempAreaCode = [self.areaCodeTextField.text substringFromIndex:1];
...
- (IBAction)registUnwind:(UIStoryboardSegue *)unwindSeuge {
...
- (void)setTheLocalAreaCode {
    ...
    self.areaCodeTextField.text = [NSString stringWithFormat:@"+%@",defaultCode];
