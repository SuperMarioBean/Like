...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Done", @"chat", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            self.title = NSLocalizedStringFromTable(@"Select Photos", @"chat", nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
                photosText = NSLocalizedStringFromTable(@"photo", @"chat", @"Used in the context: '1 photo'");
            } else {
                photosText = NSLocalizedStringFromTable(@"photos", @"chat", @"Used in the context: '3 photos'");
            }
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            self.title = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedStringFromTable(@"of", @"chat", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
        }
	} else {
		self.title = nil;
	}
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    if ([MFMailComposeViewController canSendMail]) {
                        _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                               cancelButtonTitle:NSLocalizedStringFromTable(@"Cancel", @"chat", nil) destructiveButtonTitle:nil
                                                               otherButtonTitles:NSLocalizedStringFromTable(@"Save", @"chat", nil), NSLocalizedStringFromTable(@"Copy", @"chat", nil), NSLocalizedStringFromTable(@"Email", @"chat", nil), nil];
                    } else {
                        _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                               cancelButtonTitle:NSLocalizedStringFromTable(@"Cancel", @"chat", nil) destructiveButtonTitle:nil
                                                               otherButtonTitles:NSLocalizedStringFromTable(@"Save", @"chat", nil), NSLocalizedStringFromTable(@"Copy", @"chat", nil), nil];
                    }
...
- (void)savePhoto {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedStringFromTable(@"Saving", @"chat", @"Displayed with ellipsis as 'Saving...' when an item is in the process of being saved")]];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedStringFromTable(@"Failed", @"chat", @"Informing the user a process has failed") : NSLocalizedStringFromTable(@"Saved", @"chat", @"Informing the user an item has been saved")];
    [self hideControlsAfterDelay]; // Continue as normal...
}
...
- (void)copyPhoto {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedStringFromTable(@"Copying", @"chat", @"Displayed with ellipsis as 'Copying...' when an item is in the process of being copied")]];
...
- (void)actuallyCopyPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [self showProgressHUDCompleteMessage:NSLocalizedStringFromTable(@"Copied", @"chat", @"Informing the user an item has finished copying")];
        [self hideControlsAfterDelay]; // Continue as normal...
    }
}
...
- (void)emailPhoto {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedStringFromTable(@"Preparing", @"chat", @"Displayed with ellipsis as 'Preparing...' when an item is in the process of being prepared")]];
...
- (void)actuallyEmailPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [emailer setSubject:NSLocalizedStringFromTable(@"Photo", @"chat", nil)];
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    if (result == MFMailComposeResultFailed) {
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"Email", @"chat", nil)
                                                        message:NSLocalizedStringFromTable(@"Email failed to send. Please try again.", @"chat", nil)
                              
                                                       delegate:nil
                                              cancelButtonTitle:NSLocalizedStringFromTable(@"Dismiss", @"chat", nil)
